...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    def text2ids(text, mapping, put_eos=False):
        text = text.lower()
        
        mapped_text   = []
        eos_delimiter = []
        if put_eos:
            eos_delimiter.append(target_vocab_to_int['<EOS>'])
        for line in text.split('\n'):
            mapped_text.append([mapping[x] for x in line.split()] + eos_delimiter)
        return mapped_text
    
    source_id_text = text2ids(source_text, source_vocab_to_int)
    target_id_text = text2ids(target_text, target_vocab_to_int, True)
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    cleaned = tf.strided_slice(target_data, [0, 0], [batch_size, -1], [1, 1])
    go_col  = tf.fill([batch_size, 1], go_id)
    
    return tf.concat([go_col, cleaned], 1)
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    # TODO: Implement Function
    #tf1.3
    lstm_cells = [tf.contrib.rnn.LSTMCell(rnn_size, forget_bias=1.0) for _ in range(num_layers)]
    encoder = tf.contrib.rnn.MultiRNNCell(lstm_cells)
    
    #tf1.0
    #lstm = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(rnn_size), output_keep_prob = keep_prob)
    #encoder = tf.contrib.rnn.MultiRNNCell([lstm] * num_layers)
    # creating a recurrent neural network specified by the above encoder cell
    outputs, final_state = tf.nn.dynamic_rnn(encoder, rnn_inputs, dtype=tf.float32)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    outputs_train, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, decoder_fn, dec_embed_input,
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    with tf.variable_scope("decoding", reuse=None) as decoding_scope:
        # creating the output function using lambda
        initializer = tf.truncated_normal_initializer(stddev=0.1)
        output_fn = lambda inputs: tf.contrib.layers.fully_connected(inputs,
                                                                     vocab_size, 
    ...
    with tf.variable_scope("decoding", reuse=True) as decoding_scope:
        # getting inference logits
        inference_logits = decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, go_id, eos_id,
                                                sequence_length, vocab_size, decoding_scope, output_fn, 
                                                keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    """
    # TODO: Implement Function
    enc_inputs = tf.contrib.layers.embed_sequence(input_data, vocab_size=source_vocab_size,
                                                  embed_dim=enc_embedding_size)
    # encoding the input
    enc_state = encoding_layer(enc_inputs, rnn_size, num_layers, keep_prob)
    
    # processing data for decoder
