...
def create_lookup_tables(text):
    ...
    int_to_vocab = {vocab: c for c, vocab in vocab_to_int.items()}
...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    punctuations = {'.':'||Period||', ',': '||Comma||', '"': '||Quotation_Mark||', ';': '||Semicolon||', 
                    '!': 'Exclamation_Mark||', '?': '||Question_Mark||', '(': '||Left_Parenthesses||',
...
def build_rnn(cell, inputs):
    ...
    """
    # TODO: Implement Function
    outputs, final_state = tf.nn.dynamic_rnn(cell, inputs, dtype = tf.float32,time_major=False)
...
def get_batches(int_text, batch_size, seq_length):
    ...
    input = int_text[:batch_len]
    input_batches = np.split(input.reshape(batch_size, -1), n_batches, 1)
    
    target = int_text[1:batch_len + 1]
    target[-1] = input[0]
    output_batches = np.split(target.reshape(batch_size, -1), n_batches, 1)
    
    batches_zip = list(zip(input_batches, output_batches))
    batches_zip = np.array(batches_zip)
    return batches_zip
...
def get_tensors(loaded_graph):
    ...
    tensor_initial_state = loaded_graph.get_tensor_by_name('initial_state:0')
    tensor_final_state = loaded_graph.get_tensor_by_name('final_state:0')
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    #word = np.random.choice(list(int_to_vocab.keys()), p=probabilities)
    word = np.random.choice(len(int_to_vocab), p=probabilities)
    word_choice = int_to_vocab[word]
    return word_choice
